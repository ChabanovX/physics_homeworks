...
\begin{align}
\Delta x &= v_0 t + \frac{1}{2} a t^2 \label{eq:displacement} \\
&\text{where:} \notag \\
\Delta x &\text{ is the displacement,} \notag \\
v_0 &\text{ is the initial velocity,} \notag \\
t &\text{ is the time,} \notag \\
a &\text{ is the acceleration.} \notag
\end{align}

Equation \ref{eq:displacement} shows how displacement is calculated for an object under constant acceleration.
...
In order to plot the displacement graph, let us clearly state coordinate function next.
As seen in equation \ref{eq:displacement}, the displacement depends on the initial velocity and the acceleration.
