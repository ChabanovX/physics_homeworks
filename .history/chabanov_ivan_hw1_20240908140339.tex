...
Displacement graphs are a little bit trickier, that the acceleration ones. In order to plot the displacement graph, let us clearly state coordinate function next using equation \ref{eq:displacement}.
...
But still we can not plot the graph using this functions as we have to transfer time in the future for some of them. For example, $x(t)_{[4:10]}$ should be $x(t-4)_{[4:10]}$ as the body started to move this way only from 4th second.
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Acceleration and Displacement Plots}
    \label{fig:acceleration_displacement}
